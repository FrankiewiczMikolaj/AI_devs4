...
import { CLI_FORCE, CACHE_DIR, formatLessonPath, TAGGING_CACHE_PATH } from "./config.js";
...
async function readTaggingCache(suspectCount: number): Promise<JobTaggingResult | null> {
  ...
  if (!isValidCache(cached, suspectCount)) {
    log.cache("Tagging cache stale, refreshing");
    return null;
  }
...
export async function tagSuspects(suspects: Suspect[]): Promise<JobTaggingResult> {
  if (!CLI_FORCE) {
    ...
    if (cached) {
      log.cache(`Using tagging cache: ${formatLessonPath(TAGGING_CACHE_PATH)}`);
      return cached;
    }
  } else {
    log.cache("Skipping tagging cache (--force)");
  }
  ...
  log.cache(`Wrote tagging cache: ${formatLessonPath(TAGGING_CACHE_PATH)}`);
